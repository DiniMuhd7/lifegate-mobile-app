...
export const ProfessionalService = {
  ...
  /**
   * Fetch the physician's saved AI workspace content.
   * GET /physician/workspace
   */
  async getWorkspace(): Promise<{ identity_md: string; user_md: string; persona_md: string }> {
    const response = await api.get<{
      success: boolean;
      data: { identity_md: string; user_md: string; persona_md: string };
    }>('/physician/workspace');
    if (!response.data.success) throw new Error('Failed to fetch workspace');
    return response.data.data;
  },

  /**
   * Save the physician's AI workspace content.
   * PUT /physician/workspace
   */
  async saveWorkspace(payload: {
    ...
    if (!response.data.success) throw new Error(response.data.message ?? 'Failed to save workspace');
  },
  ...
  async updateAIOutput(
    ...
    const response = await api.patch<{ success: boolean; message: string }>(
      ...
      { condition, urgency, confidence, notes, prescriptions, investigations, healthTips },
    );
    if (!response.data.success)
