...
func main() {
	...
	// (Sunday 10 AM WAT), re-engagement pushes for lapsed users (Wednesday 12 PM WAT),
	// and post-case follow-up reminders at 24 h / 3 days / 7 days.
	...
	openclawWorker.SetFollowUpScheduler(notifScheduler)
	// Let the physician handler bust the openclaw persona cache on workspace save.
	physicianHandler.SetWorkspaceInvalidator(openclawWorker)
...
// Access Portal — institutional access requests with Flutterwave payment
	...
	physicianGroup := api.Group("/physician", middleware.Auth(cfg.JWTSecret), middleware.PhysicianOnly())
	{
		...
		// AI workspace — editable persona sections (only accessible when AI mode is on)
		physicianGroup.GET("/workspace", physicianHandler.GetWorkspace)
		physicianGroup.PUT("/workspace", physicianHandler.SaveWorkspace)
