...
func (s *Service) ReviewReport(reportID, physicianID string, input ReviewInput) error {
	...
	// Schedule follow-up pushes (24 h, 3 days, 7 days) after case completion.
	if input.Action == "Completed" && s.followUpSched != nil && patientID != "" {
		patientName, condition := "there", "your recent condition"
		var n, c string
		if err := s.repo.DB().QueryRow(
			`SELECT COALESCE(u.name,''), COALESCE(d.condition,'')
			 FROM   diagnoses d
			 JOIN   users u ON u.id = d.user_id
			 WHERE  d.id = $1::uuid`, reportID,
		).Scan(&n, &c); err == nil {
			if n != "" {
				patientName = n
			}
		...
		s.followUpSched.ScheduleFollowUp(patientID, patientName, condition)
	}
