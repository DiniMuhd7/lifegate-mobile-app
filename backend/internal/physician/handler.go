...
type Handler struct {
	svc                  *Service
	workspaceInvalidator WorkspaceCacheInvalidator // optional; nil = no cache bust
}
...
// SetWorkspaceInvalidator wires in the openclaw worker so that saving a
// workspace immediately busts the in-memory persona cache for that physician.
func (h *Handler) SetWorkspaceInvalidator(inv WorkspaceCacheInvalidator) {
	h.workspaceInvalidator = inv
}
...
// GetWorkspace returns the physician's current AI workspace content.
// Only accessible when AI mode is enabled for the physician.
func (h *Handler) GetWorkspace(c *gin.Context) {
	pid := physID(c)
	ws, err := h.svc.GetWorkspace(pid)
	if err != nil {
		if err == ErrAIModeNotEnabled {
			c.JSON(http.StatusForbidden, gin.H{"success": false, "message": "AI mode is not enabled"})
			return
		}
		internalError(c, err, "GetWorkspace handler")
		return
	}
	c.JSON(http.StatusOK, gin.H{"success": true, "data": ws})
}

// SaveWorkspace upserts the physician's AI workspace content and invalidates
// the openclaw worker's persona cache so changes take effect immediately.
func (h *Handler) SaveWorkspace(c *gin.Context) {
	pid := physID(c)
	var req WorkspaceInput
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "invalid request body"})
		return
	}
	if err := h.svc.SaveWorkspace(pid, req, h.workspaceInvalidator); err != nil {
		if err == ErrAIModeNotEnabled {
			c.JSON(http.StatusForbidden, gin.H{"success": false, "message": "AI mode is not enabled"})
			return
		}
		var ve *WorkspaceValidationError
		if errors.As(err, &ve) {
			c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": ve.Error()})
			return
		}
		internalError(c, err, "SaveWorkspace handler")
		return
	}
	c.JSON(http.StatusOK, gin.H{"success": true, "message": "Workspace saved"})
}

// physID extracts the physician userID from the gin context.
func physID(c *gin.Context) string {
	v, _ := c.Get("userID")
	s, _ := v.(string)
	return s
}
