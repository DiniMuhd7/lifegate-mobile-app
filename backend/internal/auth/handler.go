...
// UpdateBasicProfile updates the authenticated user's basic demographic fields.
//
...
// @Produce      json
// @Security     BearerAuth
// @Param        body  body      object{name=string,phone=string,dob=string,gender=string}  false  "Profile fields to update"
// @Success      200   {object}  object{success=bool,message=string,data=object{user=object}}
// @Failure      400   {object}  object{success=bool,message=string}
// @Failure      401   {object}  object{success=bool,message=string}
...
func (h *Handler) UpdateBasicProfile(c *gin.Context) {
	...
	if req.Name == "" && req.Phone == "" && req.DOB == "" && req.Gender == "" {
		respond(c, http.StatusBadRequest, false, "at least one of name, phone, dob or gender must be provided", nil)
		return
	}
	user, err := h.svc.repo.UpdateBasicProfile(uid, req.Name, req.Phone, req.DOB, req.Gender)
	if err != nil {
