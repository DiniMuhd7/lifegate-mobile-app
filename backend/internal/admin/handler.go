...
//
// @Summary      List patients by registration date
// @Tags         admin
// @Produce      json
// @Security     BearerAuth
...
// ExportPatientsCSV streams a CSV of patients registered within a required
// date range, for the admin "export patients/users registration" feature.
//
// @Summary      Export patients CSV by registration date
// @Tags         admin
// @Produce      text/csv
// @Security     BearerAuth
// @Param        dateFrom  query     string  true  "Start date (YYYY-MM-DD)"
...
func (h *Handler) ExportPatientsCSV(c *gin.Context) {
	...
	adminID, _ := c.Get("userID")
	adminIDStr, _ := adminID.(string)
	h.svc.LogAction(adminIDStr, "patients.export", "user", nil,
		map[string]interface{}{"dateFrom": dateFrom, "dateTo": dateTo})
...
// ImportPatientsCSV accepts a CSV upload and bulk-updates existing patients'
// blood group, genotype, height/weight (BMI), and other test-result fields,
// matching each row to an existing patient by email. Rows with an email that
// doesn't match an existing patient are reported as errors and skipped —
// this endpoint never creates new accounts.
//
// Expected CSV header: a required "email" column, plus any of
// "blood_group", "genotype", "height_cm", "weight_kg", and any other custom
// column name (e.g. "hemoglobin", "glucose") which is stored as a test result.
//
// @Summary      Bulk-import patient clinical data via CSV
// @Tags         admin
// @Accept       multipart/form-data
// @Produce      json
// @Security     BearerAuth
...
// @Failure      400  {object}  object{success=bool,message=string}
// @Failure      500  {object}  object{success=bool,message=string}
// @Router       /admin/patients/import [post]
func (h *Handler) ImportPatientsCSV(c *gin.Context) {
	...
	for {
		...
		if err != nil {
			break // io.EOF or unrecoverable parse error ends the loop
		}
		rowNum++
		summary.TotalRows++
		...
		if updErr := h.svc.UpdatePatientFromCSVRow(email, fields); updErr != nil {
			summary.Failed++
			summary.Results = append(summary.Results, PatientImportRowResult{
				Row: rowNum, Email: email, Status: "error", Message: updErr.Error(),
			})
			continue
		}
		summary.Updated++
		summary.Results = append(summary.Results, PatientImportRowResult{
			Row: rowNum, Email: email, Status: "updated",
		})
	}

	adminID, _ := c.Get("userID")
	adminIDStr, _ := adminID.(string)
	h.svc.LogAction(adminIDStr, "patients.import", "user", nil,
		map[string]interface{}{"totalRows": summary.TotalRows, "updated": summary.Updated, "failed": summary.Failed})
