...
// ListCompletedCasesNeedingReply returns Completed cases where the most recent
// IM was sent by the patient AND the case was completed within the last 30
// minutes — the post-completion reply window.
func (r *Repository) ListCompletedCasesNeedingReply(ctx context.Context, limit int) ([]caseNeedingReply, error) {
	const q = `
		SELECT
			d.id::text,
			d.physician_id::text,
...
// physicianWorkspace holds the editable persona sections for one human physician.
type physicianWorkspace struct {
	IdentityMD string
	UserMD     string
	PersonaMD  string
}

// GetPhysicianWorkspace returns the human physician's saved workspace content.
// Returns a zero-value struct (all empty strings) when no row exists yet.
func (r *Repository) GetPhysicianWorkspace(ctx context.Context, physicianID string) (physicianWorkspace, error) {
	var ws physicianWorkspace
	err := r.db.QueryRowContext(ctx,
		`SELECT COALESCE(identity_md,''), COALESCE(user_md,''), COALESCE(persona_md,'')
		 FROM   physician_workspace
		 WHERE  physician_id = $1::uuid`,
		physicianID,
	).Scan(&ws.IdentityMD, &ws.UserMD, &ws.PersonaMD)
	if errors.Is(err, sql.ErrNoRows) {
		return physicianWorkspace{}, nil
	}
	return ws, err
}
