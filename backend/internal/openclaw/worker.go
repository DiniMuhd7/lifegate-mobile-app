...
// FollowUpScheduler schedules post-case follow-up push notifications.
// Satisfied by *notifications.Scheduler.
...
type Worker struct {
	...
	followUp   FollowUpScheduler  // optional
	agentsDir  string
...
// SetFollowUpScheduler wires in the post-case follow-up scheduler.
func (w *Worker) SetFollowUpScheduler(f FollowUpScheduler) { w.followUp = f }

// InvalidatePersonaCache removes the cached persona for a single physician ID.
// Called by the physician handler immediately after a workspace save so the
// next reply cycle loads the updated content from the DB.
// Satisfies physician.WorkspaceCacheInvalidator.
func (w *Worker) InvalidatePersonaCache(physicianID string) {
	w.personaMu.Lock()
	delete(w.personaCache, physicianID)
	w.personaMu.Unlock()
	log.Printf("[openclaw-worker] persona cache invalidated for physician %s", physicianID)
}
...
func (w *Worker) runReplyCycle(ctx context.Context) {
	...
	completedCases, err := w.repo.ListCompletedCasesNeedingReply(ctx, replyBatchLimit)
	if err != nil {
		log.Printf("[openclaw-worker] list completed cases needing reply: %v", err)
	} else {
		cases = append(cases, completedCases...)
	}
...
func (w *Worker) handleReview(ctx context.Context, c caseForReview) error {
	...
	// Schedule 24-hour, 3-day, and 7-day follow-up pushes so the patient
	// hears from us more than once if they don't re-open the app.
	if w.followUp != nil {
		w.followUp.ScheduleFollowUp(patientID, c.PatientName, c.Condition)
	}
...
// buildHumanAIReplyPrompt builds the patient-reply system prompt for a human
// physician operating in AI mode.
//
// Priority order:
//   1. Physician's saved workspace (identity_md + user_md + persona_md from DB)
//      — cached in personaCache under the physician's userID.
//   2. Generic LifeGate physician fallback when no workspace is saved yet.
//
// The platform safety rules (MDCN ethics, anti-hallucination, emergency
// escalation) are always appended after the workspace content so the physician
// cannot inadvertently remove them.
func (w *Worker) buildHumanAIReplyPrompt(c caseNeedingReply) (string, error) {
	...
	// Check cache first (keyed by physician userID, not slug).
	cacheKey := "human:" + c.PhysicianID
	w.personaMu.RLock()
	cached, ok := w.personaCache[cacheKey]
	w.personaMu.RUnlock()
	...
	if ok {
		...
		ws, err := w.repo.GetPhysicianWorkspace(context.Background(), c.PhysicianID)
		if err != nil {
			log.Printf("[openclaw-worker] workspace load error for physician %s: %v — using fallback", c.PhysicianID, err)
		}

		if ws.IdentityMD != "" || ws.UserMD != "" || ws.PersonaMD != "" {
			// Physician has customised their workspace — assemble like a real agent.
			var sb strings.Builder
			if ws.IdentityMD != "" {
				sb.WriteString(ws.IdentityMD)
				sb.WriteString("\n\n---\n\n")
			}
			if ws.UserMD != "" {
				sb.WriteString(ws.UserMD)
				sb.WriteString("\n\n---\n\n")
			}
			if ws.PersonaMD != "" {
				sb.WriteString(ws.PersonaMD)
				sb.WriteString("\n\n---\n\n")
			}
			persona = sb.String()
		} else {
			// No workspace saved — use the generic fallback.
			persona = fmt.Sprintf(
				"You are %s, a licensed medical doctor on the LifeGate platform currently operating in AI-assisted mode.\n",
				name,
			)
		}
		...
		w.personaMu.Lock()
		w.personaCache[cacheKey] = persona
		w.personaMu.Unlock()
	}

	// Always append platform safety rules so the physician cannot
	// accidentally omit them from their workspace content.
	var sb strings.Builder
	sb.WriteString(persona)
...
// buildHumanAIReviewPrompt builds the structured case-review system prompt for
// a human physician operating in AI mode.  It injects the physician's workspace
// persona at the top, then delegates to the standard review prompt builder.
func (w *Worker) buildHumanAIReviewPrompt(c caseForReview) (string, error) {
	...
	// Load persona header (same cache as reply prompt).
	cacheKey := "human:" + c.PhysicianID
	w.personaMu.RLock()
	cached, ok := w.personaCache[cacheKey]
	w.personaMu.RUnlock()
	...
	if ok {
		...
		if ws.IdentityMD != "" || ws.UserMD != "" || ws.PersonaMD != "" {
			...
			header = fmt.Sprintf(
				"You are %s, a licensed physician on the LifeGate platform operating in AI-assisted mode.\n\n",
				name,
			)
		}
		w.personaMu.Lock()
		w.personaCache[cacheKey] = header
		w.personaMu.Unlock()
	}
