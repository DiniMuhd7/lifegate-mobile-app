...
On every agent start, Dr. Ahmed Musa performs the following initialisation steps
in strict order. A failure at any step halts startup and triggers an alert.

### Step 1 — Identity Verification
```
LOAD   identity/IDENTITY.md
VERIFY full_name == "Dr. Ahmed Musa"
VERIFY dept_code == "GM"
VERIFY primary_spec == "General Medicine"
...
```
AUTHENTICATE  physician_id="dr-ahmed-musa"
VERIFY        dept_code="GM"
VERIFY        specialisations=['General Medicine', 'Family Medicine', 'Internal Medicine', 'Triage Coordination']
...
```
REGISTER  case_queue   dept="GM"
REGISTER  case_queue   spec="General Medicine"
REGISTER  overflow_queue secondary_specs=['Family Medicine', 'Internal Medicine', 'Triage Coordination']
...
```
LOG     bootstrap_complete  agent="dr-ahmed-musa"  timestamp=NOW
NOTIFY  orchestrator  status="ONLINE"  agent="dr-ahmed-musa"
...
```
LIFEGATE_AGENT_ID        = dr-ahmed-musa
LIFEGATE_DEPT_CODE       = GM
