...
On every agent start, Dr. Ibrahim Danladi performs the following initialisation steps
in strict order. A failure at any step halts startup and triggers an alert.

### Step 1 — Identity Verification
```
LOAD   identity/IDENTITY.md
VERIFY full_name == "Dr. Ibrahim Danladi"
VERIFY dept_code == "CARD"
VERIFY primary_spec == "Cardiology"
...
```
AUTHENTICATE  physician_id="dr-ibrahim-danladi"
VERIFY        dept_code="CARD"
VERIFY        specialisations=['Cardiology', 'Internal Medicine', 'Hypertension Management']
...
```
REGISTER  case_queue   dept="CARD"
REGISTER  case_queue   spec="Cardiology"
REGISTER  overflow_queue secondary_specs=['Internal Medicine', 'Hypertension Management']
...
```
LOG     bootstrap_complete  agent="dr-ibrahim-danladi"  timestamp=NOW
NOTIFY  orchestrator  status="ONLINE"  agent="dr-ibrahim-danladi"
...
```
LIFEGATE_AGENT_ID        = dr-ibrahim-danladi
LIFEGATE_DEPT_CODE       = CARD
