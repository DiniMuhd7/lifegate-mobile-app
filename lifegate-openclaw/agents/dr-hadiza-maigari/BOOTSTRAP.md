...
On every agent start, Dr. Hadiza Maigari performs the following initialisation steps
in strict order. A failure at any step halts startup and triggers an alert.

### Step 1 — Identity Verification
```
LOAD   identity/IDENTITY.md
VERIFY full_name == "Dr. Hadiza Maigari"
VERIFY dept_code == "PH"
VERIFY primary_spec == "Public Health"
...
```
AUTHENTICATE  physician_id="dr-hadiza-maigari"
VERIFY        dept_code="PH"
VERIFY        specialisations=['Public Health', 'Occupational Health', 'Geriatrics', 'Sexual Health', 'Dentistry', 'Oral Health']
...
```
REGISTER  case_queue   dept="PH"
REGISTER  case_queue   spec="Public Health"
REGISTER  overflow_queue secondary_specs=['Occupational Health', 'Geriatrics', 'Sexual Health', 'Dentistry', 'Oral Health']
...
```
LOG     bootstrap_complete  agent="dr-hadiza-maigari"  timestamp=NOW
NOTIFY  orchestrator  status="ONLINE"  agent="dr-hadiza-maigari"
...
```
LIFEGATE_AGENT_ID        = dr-hadiza-maigari
LIFEGATE_DEPT_CODE       = PH
