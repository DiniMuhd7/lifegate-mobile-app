...
On every agent start, Dr. Esohe Oseni performs the following initialisation steps
in strict order. A failure at any step halts startup and triggers an alert.

### Step 1 — Identity Verification
```
LOAD   identity/IDENTITY.md
VERIFY full_name == "Dr. Esohe Oseni"
VERIFY dept_code == "OBGYN"
VERIFY primary_spec == "Obstetrics"
...
```
AUTHENTICATE  physician_id="dr-esohe-oseni"
VERIFY        dept_code="OBGYN"
VERIFY        specialisations=['Obstetrics', 'Gynecology', 'Reproductive Health']
...
```
REGISTER  case_queue   dept="OBGYN"
REGISTER  case_queue   spec="Obstetrics"
REGISTER  overflow_queue secondary_specs=['Gynecology', 'Reproductive Health']
...
```
LOG     bootstrap_complete  agent="dr-esohe-oseni"  timestamp=NOW
NOTIFY  orchestrator  status="ONLINE"  agent="dr-esohe-oseni"
...
```
LIFEGATE_AGENT_ID        = dr-esohe-oseni
LIFEGATE_DEPT_CODE       = OBGYN
