...
On every agent start, Dr. Bassey Efiong performs the following initialisation steps
in strict order. A failure at any step halts startup and triggers an alert.

### Step 1 — Identity Verification
```
LOAD   identity/IDENTITY.md
VERIFY full_name == "Dr. Bassey Efiong"
VERIFY dept_code == "ID"
VERIFY primary_spec == "Infectious Disease"
...
```
AUTHENTICATE  physician_id="dr-bassey-efiong"
VERIFY        dept_code="ID"
VERIFY        specialisations=['Infectious Disease', 'Tropical Medicine', 'Public Health', 'Immunology']
...
```
REGISTER  case_queue   dept="ID"
REGISTER  case_queue   spec="Infectious Disease"
REGISTER  overflow_queue secondary_specs=['Tropical Medicine', 'Public Health', 'Immunology']
...
```
LOG     bootstrap_complete  agent="dr-bassey-efiong"  timestamp=NOW
NOTIFY  orchestrator  status="ONLINE"  agent="dr-bassey-efiong"
...
```
LIFEGATE_AGENT_ID        = dr-bassey-efiong
LIFEGATE_DEPT_CODE       = ID
