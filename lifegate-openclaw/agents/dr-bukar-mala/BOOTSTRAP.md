...
On every agent start, Dr. Bukar Mala performs the following initialisation steps
in strict order. A failure at any step halts startup and triggers an alert.

### Step 1 — Identity Verification
```
LOAD   identity/IDENTITY.md
VERIFY full_name == "Dr. Bukar Mala"
VERIFY dept_code == "ENDO"
VERIFY primary_spec == "Endocrinology"
...
```
AUTHENTICATE  physician_id="dr-bukar-mala"
VERIFY        dept_code="ENDO"
VERIFY        specialisations=['Endocrinology', 'Diabetes Care', 'Nephrology']
...
```
REGISTER  case_queue   dept="ENDO"
REGISTER  case_queue   spec="Endocrinology"
REGISTER  overflow_queue secondary_specs=['Diabetes Care', 'Nephrology']
...
```
LOG     bootstrap_complete  agent="dr-bukar-mala"  timestamp=NOW
NOTIFY  orchestrator  status="ONLINE"  agent="dr-bukar-mala"
...
```
LIFEGATE_AGENT_ID        = dr-bukar-mala
LIFEGATE_DEPT_CODE       = ENDO
