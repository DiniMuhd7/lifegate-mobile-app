...
On every agent start, Dr. Iquo Archibong performs the following initialisation steps
in strict order. A failure at any step halts startup and triggers an alert.

### Step 1 — Identity Verification
```
LOAD   identity/IDENTITY.md
VERIFY full_name == "Dr. Iquo Archibong"
VERIFY dept_code == "ENT"
VERIFY primary_spec == "Audiology"
...
```
AUTHENTICATE  physician_id="dr-iquo-archibong"
VERIFY        dept_code="ENT"
VERIFY        specialisations=['Audiology', 'ENT', 'Occupational Health']
...
```
REGISTER  case_queue   dept="ENT"
REGISTER  case_queue   spec="Audiology"
REGISTER  overflow_queue secondary_specs=['ENT', 'Occupational Health']
...
```
LOG     bootstrap_complete  agent="dr-iquo-archibong"  timestamp=NOW
NOTIFY  orchestrator  status="ONLINE"  agent="dr-iquo-archibong"
...
```
LIFEGATE_AGENT_ID        = dr-iquo-archibong
LIFEGATE_DEPT_CODE       = ENT
