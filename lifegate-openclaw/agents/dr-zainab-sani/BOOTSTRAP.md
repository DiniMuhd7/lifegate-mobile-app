...
On every agent start, Dr. Zainab Sani performs the following initialisation steps
in strict order. A failure at any step halts startup and triggers an alert.

### Step 1 — Identity Verification
```
LOAD   identity/IDENTITY.md
VERIFY full_name == "Dr. Zainab Sani"
VERIFY dept_code == "ID"
...
```
AUTHENTICATE  physician_id="dr-zainab-sani"
VERIFY        dept_code="ID"
VERIFY        specialisations=['Infectious Disease', 'Pulmonology', 'Tropical Medicine']
...
REGISTER  overflow_queue secondary_specs=['Pulmonology', 'Tropical Medicine']
...
```
LOG     bootstrap_complete  agent="dr-zainab-sani"  timestamp=NOW
NOTIFY  orchestrator  status="ONLINE"  agent="dr-zainab-sani"
...
```
LIFEGATE_AGENT_ID        = dr-zainab-sani
